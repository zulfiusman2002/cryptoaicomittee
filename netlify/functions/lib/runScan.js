...
const { evaluateOpenTrades, openTradesFromSignals, computeStats, computePortfolio, computeAggressiveBook } = require('./paperTrading')
...
async function runScan({ waitForWrites = false, source = 'manual' } = {}) {
  ...
  const signalResults = top5.map((candidate, i) => {
    ...
    return {
      ...
      conviction: candidate.signal.conviction,
  ...
  try {
    ...
    paperTrading = {
      ...
      aggressiveBook: computeAggressiveBook(allTrades),
