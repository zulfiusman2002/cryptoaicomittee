  ...
  await db.batch([
    ...
    },
    {
      sql: `CREATE TABLE IF NOT EXISTS paper_trades (
        ...
        strategy_version  TEXT DEFAULT 'momentum-v1',
...
async function getReadyClient() {
  ...
  if (!tablesReady) {
    ...
    try {
      await db.execute({ sql: "ALTER TABLE paper_trades ADD COLUMN strategy_version TEXT DEFAULT 'momentum-v1'", args: [] })
      console.log('[db] Migrated: added paper_trades.strategy_version')
    } catch { /* column already exists */ }
    try {
      await db.execute({ sql: "ALTER TABLE paper_trades ADD COLUMN conviction TEXT DEFAULT 'low'", args: [] })
      console.log('[db] Migrated: added paper_trades.conviction')
    } catch { /* column already exists */ }
    tablesReady = true
...
async function openPaperTrade(t) {
  ...
  await db.execute({
    sql: `INSERT INTO paper_trades (
            ...
            opened_at, status, data_completeness, strategy_version, conviction
          ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    args: [
      id, t.scan_id ?? null, t.symbol, t.verdict ?? null,
      t.entry_price, t.stop_loss ?? null, t.tp1 ?? null, t.tp2 ?? null,
      now(), 'open', t.data_completeness ?? null, t.strategy_version ?? 'pullback-v2',
      t.conviction ?? 'low'
    ]
  })
  return id
...
// Trades joined to the deterministic score they entered with
async function getTradesWithScores() {
  const db = await getReadyClient()
  const result = await db.execute({
    sql: `SELECT p.*, s.deterministic_score, s.was_downgraded
          FROM paper_trades p
          LEFT JOIN signal_results s
            ON s.scan_id = p.scan_id AND s.symbol = p.symbol
...
// Trades joined to BTC's 24h return at their entry scan (regime analysis)
async function getTradesWithBtcRegime() {
  const db = await getReadyClient()
  const result = await db.execute({
    sql: `SELECT p.*, b.return_24h AS btc_return_24h
          FROM paper_trades p
          LEFT JOIN coin_metrics b
            ON b.scan_id = p.scan_id AND b.symbol = 'BTCUSDT'
          ORDER BY p.opened_at ASC`,
    args: []
  })
  return result.rows
...
// First and last BTC price across all scans — buy-and-hold benchmark
async function getBtcBenchmarkPrices() {
  const db = await getReadyClient()
  const first = await db.execute({
    sql: `SELECT price, created_at FROM coin_metrics WHERE symbol = 'BTCUSDT' AND price IS NOT NULL ORDER BY created_at ASC LIMIT 1`,
    args: []
  })
  const last = await db.execute({
    sql: `SELECT price, created_at FROM coin_metrics WHERE symbol = 'BTCUSDT' AND price IS NOT NULL ORDER BY created_at DESC LIMIT 1`,
    args: []
  })
  return {
    first: first.rows[0] || null,
    last:  last.rows[0]  || null
  }
}

async function saveWeeklyReview(tradesAnalyzed, review) {
  const db = await getReadyClient()
  await db.execute({
    sql: `INSERT INTO weekly_reviews (id, created_at, trades_analyzed, review_json) VALUES (?,?,?,?)`,
    args: [uuid(), now(), tradesAnalyzed, JSON.stringify(review)]
  })
}

async function getLatestWeeklyReview() {
  const db = await getReadyClient()
  const result = await db.execute({
    sql: `SELECT * FROM weekly_reviews ORDER BY created_at DESC LIMIT 1`,
    args: []
  })
  ...
  return { created_at: row.created_at, trades_analyzed: row.trades_analyzed, review }
}


// All coin metrics across all scans — the raw material for forward-return
// reconstruction and the base-rate test.
async function getAllCoinMetrics() {
  const db = await getReadyClient()
  const result = await db.execute({
    sql: `SELECT scan_id, created_at, symbol, price, return_1h, return_24h,
                 volume_shock, relative_strength_btc
          FROM coin_metrics
...
module.exports = {
  ...
  getAllPaperTrades, getAllCoinMetrics,
  getTradesWithScores, getTradesWithBtcRegime, getBtcBenchmarkPrices,
  saveWeeklyReview, getLatestWeeklyReview,
  healthCheck
}
