...
function computeSignal(m) {
  ...
  const inUptrend = m.return24h > 3 && m.return24h < 25
  if (inUptrend)                                  { score += 2; reasons.push('healthy 24h momentum') }
  ...
  // ── PULLBACK ENTRY (validated t=5.5 vs base rate on logged data) ──
  // Buy the dip INSIDE an uptrend: 24h up, but short-term (1h) pulling back.
  // This is the highest-performing selection rule in shadow testing — it beats
  // both raw momentum and the base rate. The old logic rewarded positive 1h
  // (chasing), which is weaker. We now reward the pullback and only mildly
  // reward continuation.
  const isPullback = inUptrend && m.return1h !== null && m.return1h < 0
  const isContinuation = inUptrend && m.return1h !== null && m.return1h > 0.5

  if (isPullback) {
    score += 2
    reasons.push('PULLBACK: dip inside uptrend (highest-conviction entry)')
    // Volume or RS confirmation on a pullback is the strongest combo (t=5.06)
    if (m.volumeShock > 2 || (m.relStrengthBtc !== null && m.relStrengthBtc > 1)) {
      score += 1
      reasons.push('pullback confirmed by volume/RS')
    }
  } else if (isContinuation) {
    score += 1
    reasons.push('positive 1h continuation (weaker than pullback)')
  }
...
function buildResult(m, verdict, score, reason, risk, levels, dataCompleteness) {
  // Conviction tier drives position sizing in the aggressive book.
  // Higher score + pullback pattern = higher conviction = bigger simulated bet.
  const isPullbackEntry = reason.includes('PULLBACK')
  let conviction = 'low'
  if (score >= 6 && isPullbackEntry)       conviction = 'high'
  else if (score >= 5 || isPullbackEntry)  conviction = 'medium'
  else if (score >= 3)                     conviction = 'low'
  return { symbol: m.symbol, verdict, score, reason, risk, levels,
           dataCompleteness, conviction, fundingCategory: m.fundingCategory, risks: m.risks || [] }
}
