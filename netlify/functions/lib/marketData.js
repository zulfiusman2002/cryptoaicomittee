...
function computeCoinMetrics(symbol, marketRow, ohlcData, btcReturn24h, ethReturn24h) {
  ...
  // Data quality: price confirmed, derivatives unavailable
  const dataQuality = 2
...
async function fetchAllMetrics() {
  ...
  // Step 2: Skip per-coin OHLC calls — they trigger 429 rate limits on CoinGecko free tier.
  // The markets endpoint already gives us 1h and 24h returns which are the primary scoring inputs.
  // 4h and 12h return as 0 (neutral) — acceptable for MVP scoring.
  // volCompression also unavailable without OHLC — marked null.
  const ohlcBySymbol = {} // empty — OHLC skipped
