...
// Separates momentum-v1 (old) trades from pullback-v2 (new) so the live
// performance of the upgraded engine can be tracked from its first trade.
// This is the panel that tells you when v2 has earned the right to real money.
function strategyVersionSplit(trades) {
  const closed = realClosed(trades)
  const byVer = {}
  for (const t of closed) {
    const v = t.strategy_version || 'momentum-v1'
    ;(byVer[v] = byVer[v] || []).push(Number(t.pnl_pct))
  }
  const rows = Object.entries(byVer).map(([version, pnls]) => {
    const n = pnls.length
    const mean = n ? pnls.reduce((s, v) => s + v, 0) / n : null
    const sd = n > 1 ? Math.sqrt(pnls.reduce((s, v) => s + (v - mean) ** 2, 0) / (n - 1)) : null
    const se = (sd && n) ? sd / Math.sqrt(n) : null
    const tStat = (se && mean !== null) ? +(mean / se).toFixed(2) : null
    return {
      version, n,
      expectancy: mean !== null ? +mean.toFixed(3) : null,
      winRate: n ? +(pnls.filter(v => v > 0).length / n * 100).toFixed(1) : null,
      tStat,
      readyForLive: tStat !== null && tStat >= 2.0 && mean > 0 && n >= 30
    }
  }).sort((a, b) => (b.n - a.n))
  return { rows, note: 'pullback-v2 is the upgraded engine. It earns real-money consideration only when its OWN row shows n≥30 and t≥2.0 — a clean live track record on the better logic, not inherited from the old trades.' }
}

module.exports.strategyVersionSplit = strategyVersionSplit
