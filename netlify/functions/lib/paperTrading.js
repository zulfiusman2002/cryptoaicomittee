  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      if (res.status === 429 && attempt < 2) {
        // Short fixed backoff only — never the multi-second waits that time out the scan
        await new Promise(r => setTimeout(r, 500))
        continue
      }
      if (res.status !== 429) console.error(`[paper] price path HTTP ${res.status} for ${symbol}`)
      return null
    } catch (e) {
...
async function evaluateOpenTrades(db, pathFetcher = fetchPricePath, currentPrices = null) {
  ...
  // Hard wall-clock budget. Paper eval is background bookkeeping — it must NEVER
  // consume the scan's function-time budget. If we exceed this, remaining trades
  // are left open and retried next scan (every 4h). 20s leaves ample room under
  // Netlify's 26s configured / 60s hard limit for the rest of the scan pipeline.
  const BUDGET_MS = 20000
  const startedAt = Date.now()

  // Concurrency 2, no inter-item gap — we rely on the budget + short backoff instead
  const results = await throttledMap(openTrades, 2, 0, async trade => {
    if (Date.now() - startedAt > BUDGET_MS) {
      return { trade, outcome: { status: 'budget_skipped' } }
    }
  ...
  for (const { trade, outcome } of results) {
    ...
    if (outcome.status === 'budget_skipped') {
      budgetSkipped++
      continue  // leave open, retry next scan
    }
  ...
  if (budgetSkipped > 0) console.warn(`[paper] ${budgetSkipped} trades skipped (time budget) — will retry next scan`)
  return { evaluated: openTrades.length, closed, pending, budgetSkipped }
}
...
// ── Aggressive book simulation (conviction-based sizing) ──────
// Shadow-only: shows what a higher-risk book WOULD return if position size
// scaled with conviction. High-conviction pullbacks bet 3×, medium 2×, low 1×.
// This is how we honestly explore "pinch of risk for high reward" — the return
// is visible and measured BEFORE any real money is sized this way.
// Book split (user's plan): £1000 steady + £500 aggressive.
const AGGRESSIVE_CAPITAL = 500
const CONVICTION_MULTIPLIER = { high: 3, medium: 2, low: 1 }
const AGG_BASE_NOTIONAL = 50  // base unit; high-conviction = £150

function computeAggressiveBook(trades) {
  const real = (Array.isArray(trades) ? trades : []).filter(t => !isDataArtifact(t) && t.status !== 'open')
  if (!real.length) {
    return { startingCapital: AGGRESSIVE_CAPITAL, equity: AGGRESSIVE_CAPITAL, returnPct: 0,
             realized: 0, n: 0, note: 'No closed trades yet.' }
  }
  let realized = 0
  const byConviction = { high: 0, medium: 0, low: 0 }
  for (const t of real) {
    const conv = t.conviction || 'low'
    const mult = CONVICTION_MULTIPLIER[conv] || 1
    const notional = AGG_BASE_NOTIONAL * mult
    realized += notional * (Number(t.pnl_pct) / 100)
    byConviction[conv] = (byConviction[conv] || 0) + 1
  }
  const equity = AGGRESSIVE_CAPITAL + realized
  return {
    startingCapital: AGGRESSIVE_CAPITAL,
    realized: +realized.toFixed(2),
    equity: +equity.toFixed(2),
    returnPct: +(((equity - AGGRESSIVE_CAPITAL) / AGGRESSIVE_CAPITAL) * 100).toFixed(2),
    n: real.length,
    convictionBreakdown: byConviction,
    sizing: 'high-conviction £150 / medium £100 / low £50 per trade',
    note: real.length < 30
      ? `Shadow simulation over ${real.length} trades — not yet enough to trust. This is what conviction-sizing WOULD have returned; no real money is sized this way until the edge is proven.`
      : 'Shadow simulation of conviction-based sizing. Compare to the steady book before committing real capital to this risk profile.'
  }
}
...
module.exports = { evaluateOpenTrades, openTradesFromSignals, computeStats, computePortfolio, computeAggressiveBook, evaluatePath, fetchPricePath, isDataArtifact }
