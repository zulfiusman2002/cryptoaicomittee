...
const { evaluateOpenTrades, openTradesFromSignals, computeStats } = require('./lib/paperTrading')
...
exports.handler = async (event) => {
  ...
  try {
    // ── Step 1: Fetch market data + evaluate open paper trades in parallel ─
    console.log('[scan] Fetching market data + evaluating paper trades...')
    const [marketResult, paperEval] = await Promise.all([
      fetchAllMetrics(),
      evaluateOpenTrades(db).catch(e => {
        console.error('[scan] Paper trade evaluation failed:', e.message)
        return { evaluated: 0, closed: 0 }
      })
    ])
    const { metrics: allMetrics, diagnostics } = marketResult
    console.log(`[scan] Paper trades: ${paperEval.evaluated} evaluated, ${paperEval.closed} closed`)
    ...
    // ── Paper trading: lock trades for this scan's BUY signals ──
    // These must complete BEFORE the response so the UI shows them immediately.
    let paperTrading = { openTrades: [], closedTrades: [], stats: computeStats(null), tradesOpenedThisScan: 0 }
    try {
      const opened = await openTradesFromSignals(db, scanId, signalResults)
      const [openTrades, closedTrades, rawStats] = await Promise.all([
      ...
      console.error('[scan] Paper trading step failed:', e.message)
    }

    // ── Return to frontend IMMEDIATELY — remaining DB writes happen after ─
