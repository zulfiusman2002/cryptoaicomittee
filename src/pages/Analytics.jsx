...
export default function Analytics() {
  ...
  const { stats, equityCurve, outcomes, concentration, shadowBarriers, scoreVsOutcome, regime, benchmark, readiness, weeklyReview, baseRate, strategies, risk, matched, strategyVersions } = data
  ...
  return (
    <div className="container">
      ...
      {/* ── Strategy version tracking (v1 momentum vs v2 pullback) ── */}
      {strategyVersions && strategyVersions.rows?.length > 0 && (
        <Card title="Engine Version — Live Track Record" accent="#8a63d2">
          <Verdict text="pullback-v2 is the upgraded engine (buy-the-dip-in-uptrend, validated t=5.5 in shadow testing). It earns real-money consideration only when its OWN row reaches n≥30 and t≥2.0 — a clean live record, not inherited from the old momentum trades." tone="neutral" />
          <div className="table-wrap">
            <table className="scan-table">
              <thead><tr>
                <th style={{ textAlign:'left' }}>Engine version</th><th>Trades</th>
                <th>Expectancy</th><th>Win rate</th><th>t-stat</th><th>Live-ready?</th>
              </tr></thead>
              <tbody>
                {strategyVersions.rows.map(r => (
                  <tr key={r.version} style={r.version==='pullback-v2' ? { background:'rgba(138,99,210,0.08)' } : {}}>
                    <td style={{ textAlign:'left' }}>{r.version}</td>
                    <td>{r.n}</td>
                    <td className={r.expectancy>0?'td-pos':'td-neg'}>{fmtPct(r.expectancy)}</td>
                    <td>{r.winRate!==null?r.winRate+'%':'—'}</td>
                    <td className={Math.abs(r.tStat??0)>=2?'td-pos':'td-na'}>{r.tStat??'—'}</td>
                    <td className={r.readyForLive?'td-pos':'td-neg'}>{r.readyForLive?'YES':'not yet'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
