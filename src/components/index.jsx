...
export function PaperTradingSection({ paperTrading }) {
  ...
  const { openTrades = [], closedTrades = [], stats = {}, portfolio = null, aggressiveBook = null } = paperTrading

  const statusLabel = (s, t) => {
    if (s === 'expired_nodata') return 'NO DATA'
    if (s === 'expired' && t && +t.pnl_pct === 0 && +t.mfe_pct === 0 && +t.mae_pct === 0) return 'NO DATA'
    return ({ open: 'OPEN', stopped: 'STOPPED', tp1_hit: 'TP1 HIT', expired: 'EXPIRED (24h)' }[s] || s)
  }
  ...
  return (
    <div style={{ marginBottom: '2.5rem' }}>
      ...
      {/* ── Aggressive book (shadow, conviction-sized) ── */}
      {aggressiveBook && aggressiveBook.n > 0 && (
        <>
          <div className="section-label" style={{ marginTop: '1.5rem' }}>
            Aggressive Book (shadow) · £{aggressiveBook.startingCapital} · conviction-sized · {aggressiveBook.n} trades
          </div>
          <div style={{
            display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))',
            gap: '1px', background: 'var(--border)', border: '1px solid #8a63d2',
            borderRadius: '3px', overflow: 'hidden', marginBottom: '0.75rem'
          }}>
            {[
              ['Equity', fmtGBP(aggressiveBook.equity), aggressiveBook.equity >= aggressiveBook.startingCapital ? 'td-pos' : 'td-neg'],
              ['Return', fmtPct(aggressiveBook.returnPct), pnlClass(aggressiveBook.returnPct)],
              ['Realized P&L', fmtGBP(aggressiveBook.realized), pnlClass(aggressiveBook.realized)],
              ['High-conv trades', aggressiveBook.convictionBreakdown?.high ?? 0, ''],
            ].map(([label, value, cls]) => (
              <div key={label} style={{ background: 'var(--bg-card)', padding: '0.75rem 1rem' }}>
                <div className="metric-label">{label}</div>
          ...
          <div style={{ fontFamily: 'var(--mono)', fontSize: '0.62rem', color: 'var(--amber)',
            marginBottom: '1.5rem', letterSpacing: '0.04em' }}>
            ⚠ {aggressiveBook.note} Sizing: {aggressiveBook.sizing}.
          </div>
        </>
      )}
              ...
              <tbody>
                {openTrades.map(t => (
                  <tr key={t.id}>
                    ...
                    <td><span className="verdict-badge verdict-WAIT">{statusLabel(t.status, t)}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
